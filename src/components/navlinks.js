...
function ListItem({ data, section }) {
  ...
  let anchor = {
    href:
      intl.routed || intl.locale ? `/${intl.locale}${data.url}` : `${data.url}`,
    title: data.name,
  }
  return (
    <Location>
        ...
        if (location.pathname === "/" + intl.locale + "/") {
          return (
            <li>
              <AnchorLink
                to={anchor.href}
                className={section === anchor.title ? "active" : ""}
              >
          ...
          <li>
            <AnchorLink
              ...
              className={
                ...
                (!section &&
                  "/" + location.pathname.split("/")[1] === data.url &&
                  data.url)
                  ? "active"
                  : ""
              }
...
const NavLinks = () => {
  ...
  let findLastSection = useCallback(() => {
    ...
    for (let item of items) {
      ...
      if (
        section &&
        section.getBoundingClientRect().top < window.outerHeight * 0.5
      ) {
  ...
  items.forEach(function (e, i) {
    list.push(
      <ListItem
        key={[e.url, i, lastSectionOnScreen]}
        data={e}
        section={lastSectionOnScreen}
      />
    )
  })
