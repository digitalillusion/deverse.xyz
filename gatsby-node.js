...
exports.createPages = async ({ node, graphql, actions }) => {
  ...
  const blogPostsByCategory = result.data.blogMarkdownRemark.edges.reduce(
    (r, a) => {
  ...
  const languages = result.data.allSite.nodes[0].siteMetadata.i18n.map(
    i => i.language
  )
  Object.keys(blogPostsByCategory).forEach(category => {
    const blogPosts = blogPostsByCategory[category]
    languages.forEach(language => {
      const blogPostsByLanguage = blogPosts.filter(
        post => post.node.fields.language === language
      )
      blogPostsByLanguage.forEach((post, index) => {
        const previous =
          index === blogPostsByLanguage.length - 1
            ? null
            : blogPostsByLanguage[index + 1].node
      ...
      pages
        .filter(page => page.node.fields.language === language)
        .forEach(page => {
          createPage({
